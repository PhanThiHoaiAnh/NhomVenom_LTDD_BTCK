package com.example.bon.nhomvenom;
...
public class Fogot_password_Activity extends AppCompatActivity {
                ...
                firebaseAuth.sendPasswordResetEmail(edt_email_361.getText().toString()).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if(task.isSuccessful()){
                            Toast.makeText(Fogot_password_Activity.this,"Mật khẩu đã gửi vào email của bạn", Toast.LENGTH_LONG).show();
                        }else{
                            Toast.makeText(Fogot_password_Activity.this, task.getException().getMessage(), Toast.LENGTH_LONG).show();
                        }
                    }
                });
            }
